    ...
    if sfp_type.startswith('QSFP-DD') or sfp_type.startswith('OSFP'):
        ...
        for key in sorted_qsfp_data_map_keys:
            if key == 'cable_type':
                ...
                try:
                    # Note: The output of 'application_advertisement' by Accton's implementation
                    # might cause an exception when calling 'ast.literal_eval', and the exception
                    # will be handled in 'except' section.
                    appl_dict = ast.literal_eval(sfp_info_dict[key])
                    output += '{}{}:\n'.format(indent, QSFP_DD_DATA_MAP[key])
                    for code in appl_dict:
                        output += '{}{}: {} | {}\n'.format((indent * 2), code, appl_dict[code]['host_electrical_interface_id'], appl_dict[code]['module_media_interface_id'])
                except:
                    output += '{}{}: {}\n'.format(indent, QSFP_DD_DATA_MAP[key], sfp_info_dict[key])
...
def convert_dom_to_output_string(sfp_type, dom_info_dict):
    ...
    if sfp_type.startswith('QSFP'):
        ...
        if sfp_type.startswith('QSFP-DD'):
            ...
            if 'tx1power' not in dom_info_dict:
                 channel_monitor_map = EDFA_DOM_CHANNEL_MONITOR_MAP
            else:
                 channel_monitor_map = QSFP_DOM_CHANNEL_MONITOR_MAP
            output_dom += (indent + 'ChannelMonitorValues:\n')
            sorted_key_table = natsorted(channel_monitor_map)
            output_channel = format_dict_value_to_string(
                sorted_key_table, dom_info_dict,
                channel_monitor_map,
                DOM_VALUE_UNIT_MAP)
        ...
        # Channel Threshold
        #
        # The diff between 'SFP_DOM_CHANNEL_THRESHOLD_MAP' and 'QSFP_DOM_CHANNEL_THRESHOLD_MAP' is
        # that 'tx power' part is missing in the latter one. It is guessed that the code here is
        # a temporarily workaround because the underlying code is not ready.
        # TODO: Remove the comment out code snippet below and rename SFP_DOM_CHANNEL_THRESHOLD_MAP
        #       as DOM_CHANNEL_THRESHOLD_MAP
        #
        #if sfp_type.startswith('QSFP-DD'):
        ...
        if 'gain_report' in dom_info_dict:
            module_monitor_map = EDFA_DOM_MODULE_MONITOR_MAP
        else:
            module_monitor_map = DOM_MODULE_MONITOR_MAP
...
def logical_port_name_to_physical_port_list(port_name):
    if port_name.startswith("Ethernet"):
        if platform_sfputil is not None and platform_sfputil.is_logical_port(port_name):
...
# ===================== wrapper ====================================
def _wrapper_is_sfp_port(physical_port):
    NONE_SFP_TYPE = "NONE-SFP"
    if platform_chassis is not None:
        if hasattr(platform_chassis.get_sfp(physical_port),'sfp_type'):
            if platform_chassis.get_sfp(physical_port).sfp_type == NONE_SFP_TYPE:
                return False
    return True
                    ...
                    try:
                        ...
                        if xcvr_dom_threshold_info:
                            if xcvr_dom_info:
                                ...
                                xcvr_dom_info = xcvr_dom_threshold_info
                    except NotImplementedError:
...
def presence(port):
    ...
    for logical_port_name in logical_port_list:
        ...
        for physical_port in physical_port_list:
            ...
            if _wrapper_is_sfp_port(physical_port):
...
def fetch_error_status_from_platform_api(port):
    ...
    for logical_port_name in logical_port_list:
        ...
        for physical_port in physical_port_list:
            port_name = get_physical_port_name(logical_port_name, i, ganged)
            if not _wrapper_is_sfp_port(physical_port):
                errors = 'N/A'
                i += 1
                output.append([port_name, errors])
                continue

            try:
                errors = platform_chassis.get_sfp(physical_port).get_error_description()
            except NotImplementedError:
                errors = 'OK (Not implemented)'
...
def fetch_error_status_from_state_db(port, state_db):
    ...
    for port in sorted_ports:
        if logical_port_name_to_physical_port_list(port) is not None:
            physical_port = logical_port_name_to_physical_port_list(port)[0]
            if not _wrapper_is_sfp_port(physical_port):
                description = 'N/A'
...
@show.command()
@click.option('-p', '--port', metavar='<port_name>', help="Display SFP frequency for port <port_name> only")
def frequency(port):
    """Display frequency of SFP transceiver(s)"""
    logical_port_list = []
    output_table = []
    table_header = ["Port", "Frequency", "Grid space"]
    ...
    for logical_port_name in logical_port_list:
        ...
        for physical_port in physical_port_list:
            ...
            try:
                ...
                freq = api.get_current_laser_freq()
                freq_str = "{}GHz".format(int(freq))
                grid = api.get_freq_grid()
                grid_str = "{}GHz".format(grid)
                output_table.append([port_name, freq_str, grid_str])
            except:
                output_table.append([port_name, 'N/A', 'N/A'])
...
@cli.command()
@click.argument('port_name', metavar='<port_name>')
@click.argument('freq', metavar='<frequency>')
@click.argument('grid', metavar='<grid_space:75GHz|100GHz>')
def frequency(port_name, freq, grid):
    """Set frequency for SFP transceiver"""
    ganged = False
    i = 1
    ...
    for physical_port in physical_port_list:
        click.echo("Set frequency for port {} ... ".format(
        ...
        try:
            ...
            freq = int(freq) // 1000
            grid = grid.split('G')[0]
            result = api.set_laser_freq(int(freq),int(grid))
        except NotImplementedError:
            click.echo("This functionality is currently not implemented for this platform")
            sys.exit(ERROR_NOT_IMPLEMENTED)
        except:
            click.echo("The transceiver doesn't support to set frequency")
            sys.exit(ERROR_NOT_SUPPORT)
