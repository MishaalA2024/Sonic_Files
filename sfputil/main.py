...
#-----------------------------------Running the Static Port----------------------------------------
"""
STATIC_PORT_INDEX = 1  # Index of the static port

# Helper method for setting low-power mode
def set_lpmode(enable):
    logical_port = "static_port"  # You can change this to whatever logical port name you prefer
    ganged = False
    ...
    physical_port = physical_port_list[STATIC_PORT_INDEX - 1]  # Using the static port index
    click.echo("{} low-power mode for port {} ... ".format(
        "Enabling" if enable else "Disabling",
        get_physical_port_name(logical_port, STATIC_PORT_INDEX, ganged)), nl=False)
...
@lpmode.command()
def off():
    #Disable low-power mode for the static port
    set_lpmode(False)
...
@lpmode.command()
def on():
    #Enable low-power mode for the static port
    set_lpmode(True)
