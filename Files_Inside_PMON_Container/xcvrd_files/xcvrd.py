...
def middleware_decorator(func):
    def wrapper(*args, **kwargs):
        ...
        print(f"After calling {func.__name__} returned {result}")
        
        # Modify the response
        # modified_result = modify_response(result)
...
def _wrapper_get_transceiver_info(physical_port):
    if platform_chassis is not None:
        try:
            # Call to get transceiver info from the platform chassis
            return platform_chassis.get_sfp(physical_port).get_transceiver_info()
        except NotImplementedError:
            pass  # If the method isn't implemented, it simply passes
    # Fallback or default method to get transceiver info
...
def beautify_dom_info_dict(dom_info_dict, physical_port):
    ...
    if 'rx1power' in dom_info_dict:
        ...
        dom_info_dict['rxpower'] = strip_unit_and_beautify(dom_info_dict['rxpower'], POWER_UNIT)
        dom_info_dict['txbias'] = strip_unit_and_beautify(dom_info_dict['txbias'], BIAS_UNIT)
        dom_info_dict['txpower'] = strip_unit_and_beautify(dom_info_dict['txpower'], POWER_UNIT)
    ...
    if 'gain_report' in dom_info_dict:
        dom_info_dict['gain_report'] = strip_unit_and_beautify(dom_info_dict['gain_report'], GAIN_UNIT)
...
def post_port_dom_info_to_db(logical_port_name, port_mapping, table, transceiver_dom_dict, stop_event=threading.Event(), dom_info_cache=None):
    ganged_port = False
    ganged_member_num = 1
    ...
    for physical_port in physical_port_list:
        ...
        try:
            ...
            if dom_info_dict is not None:
                ...
                if transceiver_dom_dict is not None:
                    transceiver_dom_dict[physical_port] = dom_info_dict
                if 'rx5power' in dom_info_dict:
                    fvs = swsscommon.FieldValuePairs(
                ...
                elif 'rx1power' in dom_info_dict:
                    fvs = swsscommon.FieldValuePairs(
                        [('temperature', dom_info_dict['temperature']),
                         ('voltage', dom_info_dict['voltage']),
                         ('rx1power', dom_info_dict['rx1power']),
                    ...
                    # QSFP EDFA will fall to this section
                    fvs = swsscommon.FieldValuePairs(
                        [('temperature', dom_info_dict['temperature']),
                         ('voltage', dom_info_dict['voltage']),
                         ('rxpower', dom_info_dict['rxpower']),
                         ('txbias', dom_info_dict['txbias']),
                         ('txpower', dom_info_dict['txpower']),
                         ('rx_los', str(dom_info_dict['rx_los'])),
                         ('gain_report', dom_info_dict['gain_report']),
                         ('work_mode', dom_info_dict['work_mode']
                             if 'work_mode' in dom_info_dict else 'N/A')
                         ])
...
def post_port_sfp_dom_info_to_db(is_warm_start, port_mapping, xcvr_table_helper, stop_event=threading.Event()):
    ...
    for logical_port_name in logical_port_list:
        ...
        if rc != SFP_EEPROM_NOT_READY:
            post_port_dom_info_to_db(logical_port_name, port_mapping, xcvr_table_helper.get_dom_tbl(asic_index), transceiver_dom_dict, stop_event)
            ...
            if is_warm_start == False:
                notify_media_setting(logical_port_name, transceiver_dict, transceiver_dom_dict, xcvr_table_helper.get_app_port_tbl(asic_index), port_mapping, port_dict)
                transceiver_dict.clear()
                transceiver_dom_dict.clear()
        else:
...
# 3.Media Key = (Media type + lane speed)
def get_media_settings_key(physical_port, transceiver_dict, logical_port, port_dict):
    ...
    if port_dict:
        try:
            ...
            port_speed = int(port_dict[logical_port]['speed'])
            lane_speed = int(port_speed/num_lanes)
        except:
            lane_speed = 0

    media_key = get_media_key(physical_port) + '-' + str(lane_speed)

    return [vendor_key, module_key, media_key]
...
# The media setting can be derived by "connector", "rxpower" and
# "model" field in transceiver's eeprom with the following rule.
#
# 1.The "connector" field matched UNKNOWN_CONNECTOR_PATTERN,
#   it means that the media type can not be determine by the "connector"
#   directly. Take "rxpower" and "model" field for assist.
#   1-1.If "rxpower" field is not 'N/A' or "model" field has
#          key word "AOC", treat it as fiber
#   1-2.Otherwise, media type is regarded as copper.
# 2.The "connector" field matched COPPER_CONNECTOR_PATTERN will be
#   treated as "copper".
# 3.If the above criteria does not match, media type is regarded as fiber.
def derive_media_setting_from_xcvr_info(physical_port, transceiver_dict, transceiver_dom_dict, logical_port, port_dict):
    media_dict = {}
    key = 'media_type'
    value = ''

    connector_str = transceiver_dict[physical_port]['connector']

    if connector_str in UNKNOWN_CONNECTOR_PATTERN:
        rx_power_key = 'rx1power'

        # For QSFP-DD DAC cable, transceiver_dom_dict is None
        if physical_port in transceiver_dom_dict and rx_power_key in transceiver_dom_dict[physical_port] and transceiver_dom_dict[physical_port][rx_power_key] != 'N/A':
            value = 'fiber'
        else:
            vendor_pn_str = transceiver_dict[physical_port]['model']
            if 'AOC' in vendor_pn_str:
                value = 'fiber'
            else:
                value = 'copper'
    elif connector_str in COPPER_CONNECTOR_PATTERN:
        value = 'copper'
    else:
        value = 'fiber'

    # If the port speed is more than 40G, it should set media type as backplane for optics.
    # If the port speed is less than 40G and there is CDR supported, it should set media type as backplane.
    # If the port speed is less than 40G and there is no CDR supported, it should set media type as fiber.
    # For now, there is no port speed information passed into this function.
    # For as9716, as9726 and as9736, the most of the cases in the customer's fields will use the
    # transceiver with port speed larger than 40G. So a quick workaround is to simply set the media
    # type as backplane when the media is determined as fiber.
    try:
        port_speed = int(port_dict[logical_port]['speed'])
    except:
        port_speed = 0

    if value == 'fiber' and (port_speed >= 40000 or (port_speed < 40000 and 'CDR present' in transceiver_dict[physical_port]['ext_identifier'])):
            value = 'backplane'
...
def get_media_key(physical_port):
    ...
    is_copper = _wrapper_is_copper(physical_port)
...
# According to the eeprom data read from transceiver,
# determine the media setting on the port and write to PORT_TABLE in APP_DB.
#
# The media setting can be determined by the following two ways in order:
# 1.If pre-defined media_settings.json, generate the key from transceiver info
#   and search related configuration from this json file.
#
# 2.Derive the media setting from the "connector", "rxpower" and "model"
#   field in transceiver info.
def notify_media_setting(logical_port_name, transceiver_dict, transceiver_dom_dict,
                         app_port_tbl, port_mapping, port_dict):
    ...
    for physical_port in physical_port_list:
        ...
        # Derive media type from transceiver eeprom information and port speed
        if platform_name == "x86_64-accton_as9726_32d-r0" or platform_name == "x86_64-accton_as9716_32d-r0" or platform_name == "x86_64-accton_as9736_64d-r0":
            media_type_dict = derive_media_setting_from_xcvr_info(physical_port, transceiver_dict, transceiver_dom_dict, logical_port_name, port_dict)

            if len(media_type_dict):
                value = swsscommon.FieldValuePairs([('media_type', media_type_dict['media_type'])])
                app_port_tbl.set(port_name, value)

        # Get media settings from json file
        key = get_media_settings_key(physical_port, transceiver_dict, logical_port_name, port_dict)
...
class CmisManagerTask:
    ...
    def task_worker(self):
        ...
        while not self.task_stopping_event.is_set():
            ...
            #self.log_notice("**********************")
            #self.log_notice(self.port_dict)
            #self.log_notice("**********************")
            #self.log_notice(self.cmis_port)
            #self.log_notice("**********************")
            for lport in self.cmis_port:
                ...
                pport = int(self.port_dict[lport].get('index', "-1"))
                speed = int(self.port_dict[lport].get('speed', "0"))
                lanes = self.port_dict[lport].get('lanes', "").strip()
                if pport < 0 or speed == 0 or len(lanes) < 1:
                    continue
                self.log_notice("*******************")
                self.log_notice(pport)
                self.log_notice("*******************")
                # Desired port speed on the host side
                host_speed = speed

                parent_port = self.port_dict[lport]['parent_port']
                ...
                try:
                    ...
                    if lport == parent_port and api.is_coherent_module():
                       if 'tx_power' not in self.port_dict[lport]:
                           self.log_notice("dentro tx_power")
                           self.port_dict[lport]['tx_power'] = self.get_configured_tx_power_from_db(lport)
                       if 'laser_freq' not in self.port_dict[lport]:
                           self.log_notice("dentro freq")
                           (self.port_dict[lport]['laser_freq'], self.port_dict[lport]['grid_space']) = self.get_configured_laser_freq_from_db(lport)
                ...
                try:
                    ...
                    self.log_notice("state"+ str(state))
                    if state == self.CMIS_STATE_INSERTED:
                        self.log_notice(" dentro INSERTED ")
                        self.log_notice(lport)
                        self.log_notice(self.port_dict[lport])
                        ...
                        if lport == parent_port and api.is_coherent_module():
                           ...
                           if 0 != tx_power and tx_power != api.get_tx_config_power():
                              if True != self.configure_tx_output_power(api, lport, tx_power):
                        ...
                        if lport == parent_port and api.is_coherent_module():
                           ...
                           grid = self.port_dict[lport]['grid_space']
                           ...
                           if (0 != freq and freq != api.get_laser_config_freq()) or \
                               ...
                               if self.verify_config_laser_frequency(api, lport, freq, grid) == True:
                                   need_update = True
                                   self.log_notice("set need update TRUE")

                        if not need_update:
                            # No application updates
                            self.log_notice("dentro need update")
                        ...
                        self.port_dict[lport]['apsel'] = api.get_active_apsel_hostlane()

                        # D.2.2 Software Deinitialization
                        # set low power mode would be delayed when cmis hw initializing is in progress
                        if lport == parent_port and self.is_cmis_hw_initializing(api, all_host_lanes):
                            self.log_notice("{}: delay SW init due to HW init is executing".format(lport))
                            continue
                        ...
                        self.log_notice("DP_DEINIT")
                        self.log_notice(lport)
                        self.log_notice(self.port_dict[lport])
                        if lport != parent_port:
                        ...
                        if self.test_datapath_state(api, host_lanes, ['DataPathInit', 'DataPathDeInit', 'DataPathTxTurnOn', 'DataPathTxTurnOff']):
                            self.log_notice("{}: wait datapath finish transient state.".format(lport))
                            continue
                        api.set_lpmode(True)
                        ...
                        # For ZR module, configure the laser frequency when Datapath is in Deactivated state
                           ...
                           if 0 != freq and 0 != grid:
                                if True != self.configure_laser_frequency(api, lport, freq, grid):
                                   self.log_error("{} failed to configure laser frequency {} GHz grid space {} GHz".format(lport, freq, grid))
                                else:
                                   self.log_notice("{} configured laser frequency {} GHz grid space {} GHz".format(lport, freq, grid))
                        ...
                        if not self.test_config_active(api, host_lanes, appl):
                            if (expired is not None) and (expired <= now):
                                self.log_notice("{}: timeout for 'Config Active'".format(lport))
                                self.reset_cmis_init(lport, retries + 1)
                            else:
                                api.apply_application(host_lanes)
                                self.log_notice("{}: re-apply application".format(lport))
                            continue
                        ...
                        if lport == parent_port:
                            ...
                            if new_apsel_dict != self.port_dict[lport]['apsel']:
                                transceiver_dict = {}
                                asic_index = self.port_mapping.get_asic_id_for_logical_port(lport)
                                # post_port_sfp_info_to_db will update all of the breakout ports
                                # belong to the same parent port
                                post_port_sfp_info_to_db(lport, self.port_mapping, self.xcvr_table_helper.get_intf_tbl(asic_index),
...
class DomInfoUpdateTask(object):
    ...
    def task_worker(self, y_cable_presence):
        ...
        sel_for_state, asic_context_for_state = port_mapping.subscribe_port_update_event(['STATE_DB'])
        ...
        while not self.task_stopping_event.wait(DOM_INFO_UPDATE_PERIOD_SECS):
            ...
            port_mapping.handle_port_update_event(sel_for_state, asic_context_for_state, self.task_stopping_event, helper_logger, self.on_port_insert_remove)
            ...
            for logical_port_name in logical_port_list:
                ...
                if not sfp_status_helper.detect_port_in_error_status(logical_port_name, self.xcvr_table_helper.get_status_tbl(asic_index)):
                    post_port_dom_info_to_db(logical_port_name, self.port_mapping, self.xcvr_table_helper.get_dom_tbl(asic_index), None, self.task_stopping_event, dom_info_cache=dom_info_cache)
                    ...
                    if y_cable_presence[0] is True:
                        y_cable_helper.check_identifier_presence_and_update_mux_info_entry(self.xcvr_table_helper.get_state_db(asic_index), mux_tbl, asic_index, logical_port_name)
    ...
    def on_port_insert_remove(self, port_change_event):
        if port_change_event.event_type == port_mapping.PortChangeEvent.PORT_INSERT:
            logical_port_name = port_change_event.port_name
            physical_port_list = self.port_mapping.logical_port_name_to_physical_port_list(logical_port_name)
            if physical_port_list is None:
                helper_logger.log_error("No physical ports found for logical port '{}'".format(logical_port_name))
                return PHYSICAL_PORT_NOT_EXIST

            for physical_port in physical_port_list:
                _wrapper_refresh_xcvr_api(physical_port)
...
class SfpStateUpdateTask(object):
    ...
    def task_worker(self, stopping_event, sfp_error_event, y_cable_presence):
        ...
        state = STATE_INIT
        sel, asic_context = port_mapping.subscribe_port_config_change()
        sel_for_appdb, asic_context_for_appdb = port_mapping.subscribe_port_update_event(['APPL_DB'])
                        ...
                        for logical_port in logical_port_list:
                            ...
                            if value == sfp_status_helper.SFP_STATUS_INSERTED:
                                ...
                                if read_eeprom_state == READ_EEPROM_STATE_INSERTED:
                                    helper_logger.log_info("Got SFP inserted event on {}".format(logical_port))
                                ...
                                rc = post_port_sfp_info_to_db(logical_port, self.port_mapping, self.xcvr_table_helper.get_intf_tbl(asic_index), transceiver_dict, threading.Event(), self.sfp_insert_events)
                                if rc == SFP_EEPROM_NOT_READY:
                                    # If still failed to read EEPROM, put it to retry set
                                    if read_eeprom_state == READ_EEPROM_STATE_LAST_TRY:
                                        self.retry_eeprom_set.add(logical_port)
                                    continue
                                else:
                                    post_port_dom_info_to_db(logical_port, self.port_mapping, self.xcvr_table_helper.get_dom_tbl(asic_index), transceiver_dom_dict)
                                    post_port_dom_threshold_info_to_db(logical_port, self.port_mapping, self.xcvr_table_helper.get_dom_tbl(asic_index))
                                    # Update the media setting information until the eeprom data is read at the last time.
                                    if read_eeprom_state == READ_EEPROM_STATE_LAST_TRY:
                                        notify_media_setting(logical_port, transceiver_dict, transceiver_dom_dict, self.xcvr_table_helper.get_app_port_tbl(asic_index), self.port_mapping, self.port_dict)
                                    transceiver_dict.clear()
                                    transceiver_dom_dict.clear()
                                    if hasattr(platform_sfputil, 'qsfp_init') == True:
                                        physical_port_list = port_mapping.logical_port_name_to_physical_port_list(logical_port)
                                        for physical_port in physical_port_list:
                                            platform_sfputil.qsfp_init(physical_port)
                            elif value == sfp_status_helper.SFP_STATUS_REMOVED:
                                helper_logger.log_info("Got SFP removed event on {}".format(logical_port))
                                update_port_transceiver_status_table(
    ...
    def port_speed_change_event_handler(self, port_change_event):
        ...
        if notify_media is True:
            ...
            notify_media_setting(lport, transceiver_dict, transceiver_dom_dict, self.xcvr_table_helper.get_app_port_tbl(asic_id), self.port_mapping, self.port_dict)
        ...
        if sfp_status:
            ...
            if _wrapper_get_presence(port_change_event.port_index) and read_eeprom:
                ...
                if rc == SFP_EEPROM_NOT_READY:
                    ...
                    post_port_dom_info_to_db(port_change_event.port_name, self.port_mapping, dom_tbl, transceiver_dom_dict)
                    post_port_dom_threshold_info_to_db(port_change_event.port_name, self.port_mapping, dom_tbl)
                    notify_media_setting(port_change_event.port_name, transceiver_dict, transceiver_dom_dict, self.xcvr_table_helper.get_app_port_tbl(port_change_event.asic_id), self.port_mapping, self.port_dict)
            else:
    ...
    def retry_eeprom_reading(self):
        ...
        for logical_port in self.retry_eeprom_set:
            helper_logger.log_notice("{}: retry eeprom".format(logical_port))
            asic_index = self.port_mapping.get_asic_id_for_logical_port(logical_port)
...
class DaemonXcvrd(daemon_base.DaemonBase):
    ...
    def load_media_settings(self):
        ...
        platform_name = device_info.get_platform()
    ...
    def run(self):
        ...
        #self.log_notice("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
        #self.log_notice(str(port_mapping_data))
        #self.log_notice("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
